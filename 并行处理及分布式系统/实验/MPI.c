...
typedef double vect_t[DIM];  /* 位置、速度等的向量类型 */
...
int main(int argc, char *argv[]) {
	...
	if (rank == 0) {
		...
		MPI_Request request;
		MPI_Iscatterv(all_particles, sendcounts, displs, particle_type, curr, local_n, particle_type, 0, MPI_COMM_WORLD,
		              &request);
	...
	for (step = 1; step <= n_steps; step++) {
		...
		if (step % output_freq == 0) {
			...
			MPI_Request request;
			MPI_Igatherv(curr, local_n, particle_type, all_particles, recvcounts, displs, particle_type, 0, MPI_COMM_WORLD,
			             &request);
			MPI_Wait(&request, MPI_STATUS_IGNORE);
